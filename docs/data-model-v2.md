...
Status: all six architectural requirements and the implementation conventions are approved. Tranche 0 and the empty four-table Tranche 1A foundation are implemented. Separately numbered implementation Decision 6 approves the boundary between Tranche 2A source-evidence quarantine and Tranche 2B source-backed authority drafting. Decision 7 approves a version-controlled manifest plus pluggable content-addressed artifact-custody contract. Decision 8 approves six logical Tranche 2A evidence boundaries without fixing a physical table count. Both earlier physical Tranche 1 proposals (45 tables and 7 tables) are rejected and superseded in Git history. The authoritative Tranche 1A DDL is migration `004_tranche_1a_foundations.sql`; its retained [proposal](schema/tranche-1-foundations.proposed.sql) is a byte-identical audit artifact. No Tranche 2 physical schema, importer implementation, custody provider, substantive data, migration, API, frontend, commit, or push is yet authorized.
...
| Provenance | What was retrieved or derived, and what later supports and verifies a claim? | retrieval evidence, retained artifacts, parser runs, unverified candidates, reviews, publication and verification |

## Logical cross-tranche schema catalog

This remains the cross-tranche logical catalog. It is not a claim that all structures belong in one migration. The Tranche 1A document and proposed executable DDL supersede the abbreviated foundation descriptions below. Decision 6 also supersedes any description below that conflates a retrieval location, retrieval evidence or captured artifact with legal-document identity, officiality, representation, translation, currency, consolidation, binding force or legal effect. The exact Tranche 2A design is now specified for approval in [Tranche 2A source-evidence quarantine](schema-v2-tranche-2a.md); it is not a production migration. Tranche 2B physical names remain undecided.

SQLite uses `INTEGER` IDs/booleans and canonical `TEXT` codes, dates, and timestamps. Decision 7 fixes SHA-256 over the precisely identified stored byte sequence for raw artifacts; raw-byte hashing performs no Unicode, newline, markup or filename normalization. In the pending Tranche 2A proposal, `retrieved_body` is the exact GET response-body octets after HTTP transfer framing and before content-coding decompression; decoding is a separate processing run. Its version-1 manifest canonicalization is pinned by independently supplied golden strings and hashes. Later structured legal-content and policy hashes still require their own versioned field order, null representation, normalization and recomputation tests before implementation. Foreign keys and recursive triggers are enabled and asserted on writers. Historical records are versioned or deprecated, not silently deleted. Derived values are labelled as derived.
...
**Tranche 2A evidence substrate:** Decision 8 separates six logical records: `retrieval location -> retrieval event -> artifact identity -> artifact custody -> processing run -> unverified candidate`. This is the successful evidence path, not a requirement that every attempt reaches every stage. Manifest v1 is GET-only and records a closed representation profile, exact conditional validator/basis, supported negotiation fields, Content-Encoding and Vary. A processing run may emit zero or more derived artifacts using the same exact-byte identity mechanism, with explicit derivation lineage that must be acyclic and ultimately grounded in a retained retrieval. These are separate append-only or correction-safe records. Identical content may deduplicate stored bytes but never distinct retrieval events, processing runs or candidate occurrences. A successful retrieval remains successful when a separately recorded processing run fails. Custody occurrence time is distinct from receipt-sequence knowledge order, so later-discovered historical custody facts cannot retroactively change the evidence boundary used to accept an earlier run. A URL, retrieval metadata, or hostile extraction candidate establishes no authoritative issuer/jurisdiction, legal-document identity, officiality, currency, consolidation, binding force, legal status, or legal effect. “Retained artifact” is shorthand for an artifact identity with qualifying custody under an explicit event-time and knowledge-sequence projection. Versioned Git manifests form the pilot intake ledger rather than a seventh legal-evidence concept, while exact content-addressed bytes reside in an approved interchangeable custody backend under Decision 7. SQLite is a deterministic projection of schema migrations and structurally validated evidence bundles, not the only durable copy. The pending physical proposal uses nine tables: the six core records, normalized redirect/output children, and one technical receipt needed for deterministic sequence, provenance and idempotency. Its fixed-function importer contract binds the declared manifest path to the actual reviewed-root-relative input, completes all semantic preflight before `BEGIN`, and compares every persisted field and relationship after import and before a no-op.
...
5. Implementation Decision 6 is approved: source-evidence capture and legal-authority drafting are separate trust boundaries. It is numbered independently from the six architectural requirements. Decision 6 approves no Tranche 2 tables, fields, state codes, storage backend, importer or substantive data.
6. Implementation Decision 7 is approved: immutable, versioned Git manifests describe pilot intake; SHA-256 identifies precisely defined exact raw bytes held by an interchangeable approved custody backend; storage identity is independent of location; and SQLite is a reproducible projection. A Git-held artifact requires explicit redistribution, sensitivity, size and effectively-permanent-retention clearance. Otherwise use approved restricted durable storage or record `observed_not_retained`; however, manifest v1 rejects every personal-data-bearing artifact, and restricted custody is not a bypass. Deterministic import is network-free, transactional, idempotent and fail-closed. This approves a contract, not a provider, manifest schema, importer, table or corpus.
7. Implementation Decision 8 is approved: Tranche 2A has six logical evidence boundaries—retrieval location, retrieval event, artifact identity, artifact custody, processing run and unverified candidate occurrence. Artifact identity is separate from changing custody; derived outputs reuse exact-byte identity with explicit lineage; distinct events/runs/candidates are not deduplicated; and correction, withdrawal, relocation and tombstoning preserve history. The manifest is a transport/provenance envelope, and an import receipt is technical infrastructure. This approves no exact fields, physical table count or DDL.
...
2A. Source-evidence quarantine: the six Decision 8 logical boundaries cover retrieval locations and events, exact artifact identities, separate custody history, grounded processing runs and explicitly unverified candidate occurrences. Versioned Git manifests and pluggable content-addressed custody follow Decision 7. The pending proposal chooses a one-time sequence-1 bundle bootstrap tied to trusted runtime identity; this is not hidden seed data, and attribution remains distinct from authorization. Automation is limited to a segregated ingestion surface; authoritative issuer/jurisdiction, officiality, legal identity/status/effect, authority, review and publication remain unreachable even when a hostile candidate merely suggests one of those concepts.
...
Require migration checksums/edit detection; fresh install, upgrade from frozen 001–003 through the current chain, and upgrade from a database already at 004; transactional rollback and no-op reruns; preservation or explicit unresolved mapping of every legacy row/link; UPDATE/DELETE rejection for events, evidence and authority versions, verification and review history; clean integrity/foreign-key checks; tests for event boundary `as_of`, publication gates, stale reviews, translation warnings, coverage states, comparison cardinality, ANY/ALL/intersection behavior, compatibility/search parity, and passing `npm run check`. Atomicity is a legal/editorial assertion supported by workflow, not something SQLite alone proves.

## Approval boundary

The naming conventions, fail-closed publication policy, query-time status derivation, review/freshness architecture, and empty four-table Tranche 1A foundation are approved and implemented. The Decision 6 split between Tranche 2A evidence quarantine and Tranche 2B authority drafting, Decision 7 hybrid manifest/artifact-custody contract, and Decision 8 six logical evidence boundaries are approved. Exact fields and state codes, manifest schema, repository eligibility thresholds and rights-attestation process, artifact-backend adapter, importer, custody-provider selection, physical schema, implementation and pilot corpus each require later approval.
