...
Status: all six architectural requirements and the implementation conventions are approved. Tranche 0, the empty four-table Tranche 1A foundation, and the empty nine-table Tranche 2A source-quarantine schema are implemented. Separately numbered implementation Decisions 6–8 approve the 2A/2B trust boundary, versioned-manifest and content-addressed-custody contract, and six logical evidence boundaries. [Decision 9](tranche-2a-importer-custody.md) approves the fixed-function importer and controlled-pilot custody architecture as proposed at commit `ee3ccd777849c7ffcad1efe5d5d9170c15171a65`; its [D9.0 contract freeze](d9-0-contract-freeze.md) is now proposed for separate approval. Both earlier physical Tranche 1 proposals (45 tables and 7 tables) are rejected and superseded in Git history. Migration `004_tranche_1a_foundations.sql` is byte-identical to its retained [Tranche 1A proposal](schema/tranche-1-foundations.proposed.sql). Migration `005_tranche_2a_source_quarantine.sql` is byte-identical to the approved [Tranche 2A DDL](schema/tranche-2a-source-quarantine.proposed.sql). Migration 005 implements schema only: its nine tables are empty, no substantive evidence or legal data was imported by this milestone, and the importer, custody adapters, authentication/authorization bindings, operational controls, and Tranche 2A API/frontend integration remain unimplemented. Neither Decision 9 nor the D9.0 proposal authorizes operation. Schema implementation is not authorization for public or production use.
...
8. Implementation Decision 9 approves the fixed-function, network-free importer and controlled-pilot custody architecture as proposed at commit `ee3ccd777849c7ffcad1efe5d5d9170c15171a65`: reviewed Git manifests, a restricted local content-addressed store outside Git, independently verified backup, trusted runtime identity mapping, a disposable candidate database, and independently verified atomic promotion. This is design approval only. The proposed D9.0 freeze now makes that boundary reviewable through closed version-1 runtime, 19-role identity-binding, handoff/seal, bootstrap/recovery, custody, clearance, journal, logical-state, and result contracts; total state matrices; explicit digest profiles; deterministic ordering; exact limits; and synthetic fixtures. Those files describe required messages and later controls, not implemented security guarantees. No D9 executable tranche, bootstrap ceremony, credential, artifact custody operation, or evidence import is authorized or implemented; D9.0 awaits separate approval.
...
2A. **Physical schema implemented, empty:** migration 005 creates the six Decision 8 logical boundaries as nine `STRICT` tables, with 27 indexes and 27 triggers. The approved manifest/importer contract chooses a one-time sequence-1 bundle bootstrap tied to trusted runtime identity; this is not hidden seed data, and attribution remains distinct from authorization. The bootstrap, importer, custody adapter, and segregated automation surface are not implemented, so every 2A table remains empty. Authoritative issuer/jurisdiction, officiality, legal identity/status/effect, authority, review and publication remain unreachable even when a future hostile candidate merely suggests one of those concepts. Approved Decision 9 specifies an evidence-empty bootstrap bundle followed by a one-raw-document bundle, using a network-free importer, restricted local content-addressed custody outside Git, independent candidate-database verification, and no public/application path. D9.0 has only proposed schemas, registries, fixtures, and an offline validator; no executable D9 tranche is implemented, and all later implementation remains gated by the numbered tranches.
...
The naming conventions, fail-closed publication policy, query-time status derivation, review/freshness architecture, empty four-table Tranche 1A foundation, exact Tranche 2A physical schema, manifest contract, and Decision 9 architecture are approved. Migration 005 implements only the empty 2A schema. Decision 9 approves the design—but does not implement or authorize operation of—the repository-eligibility procedure, local artifact adapter, trusted identity bindings, fixed importer/write-isolation boundary, recovery controls, or one-document pilot profile. The proposed D9.0 machine contracts are the next separate approval boundary and are not evidence that those controls operate. Those controls, any pilot corpus, authority drafting, review/publication, API, frontend, and public or production use still require later implementation and explicit approval. No substantive evidence or legal data was imported by migration 005.
