...
Status: approved physical schema and manifest contract. Migration `005_tranche_2a_source_quarantine.sql` implements the DDL byte-for-byte; the retained [approved DDL](schema/tranche-2a-source-quarantine.proposed.sql) remains an audit artifact guarded by automated equality checks. The [bundle JSON Schema](schema/tranche-2a-evidence-bundle-v1.schema.json) and [offline validator](schema/validate-tranche-2a.mjs) remain the approved contract and adversarial design harness. Only the empty physical schema is implemented: all nine tables contain zero rows, and no importer, collector, artifact adapter, authenticated-principal mapping, operational control, evidence, legal data, API, export, search, or frontend integration was added. [Decision 9](tranche-2a-importer-custody.md) is the approved operational design for the missing fixed-function importer and controlled-pilot custody profile as proposed at commit `ee3ccd777849c7ffcad1efe5d5d9170c15171a65`, but it remains unimplemented. Its [D9.0 contract freeze](d9-0-contract-freeze.md) now proposes the closed runtime/identity/handoff/bootstrap-and-recovery/custody/clearance/journal/state/result messages, classification and digest registries, exact limits, synthetic fixtures, and offline validator for separate approval. Those contracts describe later trust boundaries; they do not implement or attest to them. Migration implementation and design approval are not authorization for public or production use.
...
The approved production-importer contract, not implemented here, requires every step through preflight to complete before a database transaction starts:
...
These tests validate the approved schema/contract and a design-only import simulation. They do not claim that a production importer, collector, parser, artifact provider, authentication system, backup, or access-control implementation exists.
...
Still unimplemented at the current boundary are production authentication/authorization, storage commit coordination and orphan reconciliation, external backup/restore and deletion-aware reconstruction, artifact rights/privacy decisions, robust secret/personal-data/malware scanning, collector network controls, parser sandboxing, and the later type-safe event/run qualification workflow. Approved Decision 9 specifies the pilot design for the importer, runtime mapping, local custody, recovery, scanning and clearance portions without implementing them. Proposed D9.0 closes their message shapes—including 19 role bindings with a distinct recovery authority, a recovery permit separate from the replay certificate, exact bootstrap/document seal cardinality, total operational matrices, and deterministic payload profiles—but none of those operational controls exists yet. Production hosted custody, processing and event/run qualification remain later work. More complex locator profiles are future-format work, not a version-1 prerequisite. The standalone Tranche 2A harness tests its synthetic adapter and simulated importer only; the D9.0 harness validates contracts and cannot prove runtime isolation or custody guarantees.
...
- Approve D9.0's proposed repository-eligibility and human rights/privacy contract, then implement and security-review it; every prospective artifact still requires an independent decision, and a schema-valid synthetic clearance is not that decision.
