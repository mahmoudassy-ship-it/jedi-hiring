...
Still unresolved at the implemented boundary are production authentication/authorization, storage commit coordination and orphan reconciliation, external backup/restore and deletion-aware reconstruction, artifact rights/privacy decisions, robust secret/personal-data/malware scanning, collector network controls, parser sandboxing, and the later type-safe event/run qualification workflow. Proposed Decision 9 supplies an approval-ready pilot design for the importer, runtime mapping, local custody, recovery, scanning and clearance portions without implementing them; production hosted custody, processing and event/run qualification remain later work. More complex locator profiles are future-format work, not a version-1 prerequisite. The standalone harness tests its synthetic adapter and simulated importer only.
