...
Current D9.3.1 note: the [synthetic primary-custody implementation candidate](d9-3-1-primary-custody-runtime.md) now awaits separate review. Earlier “next step” wording records the prior state; it does not imply that the candidate is approved, activated, or permitted to accept evidence. D9.3.1 does not invent the unavailable recovery authority or claim that sequential test inventory reads are atomic; therefore recovery appends and backup-dependent completion remain unreachable.
...
- Separately review the synthetic, unactivated [D9.3.1 primary-custody implementation candidate](d9-3-1-primary-custody-runtime.md) against the approved [D9.3.0 custody-durability contract freeze](d9-3-0-custody-durability-contracts.md). It does not alter D9.0.1, execute recovery, accept evidence, or authorize runtime use. The remaining operational Decision 9 sequence still requires real ceremony authorization, authenticated-principal activation, full D9.1/D9.2/D9.3 process integration, clearance procedure, D9.4 restriction/tombstone controls, D9.5 backup/restore tests, and durable operational audit before accepting any evidence.
