...
- Review the proposed [D9.3.0 custody-durability contracts](d9-3-0-custody-durability-contracts.md) as the next approval boundary; they are the separately versioned extension required by the frozen D9.0.1 policy and do not alter it. Only a later D9.3.1 tranche may implement primary custody and conservative crash-state classification; the extension authorizes no recovery action. The remaining operational Decision 9 sequence still requires real ceremony authorization, authenticated-principal activation, D9.1/D9.2 process integration, local custody adapter, clearance procedure, D9.4 restriction/tombstone controls, D9.5 backup/restore tests, and durable operational audit before accepting any evidence.
