...
Current D9.4 note: the [D9.4.0 restriction/deletion contract freeze](d9-4-restriction-deletion-contracts.md) is approved at reviewed commit `7db408fb0f9083f4a17c89cbaf1cb1a06bc44416` for its design-only boundary. Its authority roster depends on a production-verified, exact D9.0.1 19-role generation plus one separately versioned, design-only human binding used only as the second external roster adopter; it does not weaken human gates or grant operational authority. Approval leaves the fingerprinted `design_only_contract_freeze` state and contract root unchanged, creates no restriction or deletion effect, and does not establish accepted evidence or verified legal authority. The [D9 recovery-projection resolver contract](d9-recovery-projection-resolvers.md) revision `1.0.0` is approved as a design-only contract freeze at reviewed commit `d4f21787ebc4b99e032a9a2966cc828366159447`; approval is external to its fingerprinted root. Its [1.1.0 durability extension](d9-recovery-resolver-durability-extension.md) is approved as a design-only contract freeze at reviewed commit `29157dd584c81ed421dfa52313f0763ce2e0b568`, preserving that root while adding protected checkpoint and durable assessment-link semantics. The additive [checkpoint progression correction 1.2.0](d9-recovery-checkpoint-progression-v1-2.md) is approved as a design-only contract freeze at reviewed commit `ec83429ad3b9f77d655970f96d0be054f345a60a`; it permits only receipt-proven progression from an attested pre-append state to the exact authorized post-append state and forbids unexplained catch-up. Resolver outputs are technical recovery evidence only and grant no runtime or recovery authority, evidence acceptance, legal verification, deletion, restoration, publication eligibility, operational permission, or D9.5 authority. D9.4.1 remains a synthetic, unactivated implementation candidate under v1.1/v1.2 reconciliation and awaits separate approval; D9.5 remains unimplemented and separately reviewed.
