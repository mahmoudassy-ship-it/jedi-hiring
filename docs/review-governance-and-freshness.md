...
Current D9.4 note: the [D9.4.0 restriction/deletion contract freeze](d9-4-restriction-deletion-contracts.md) is approved at reviewed commit `7db408fb0f9083f4a17c89cbaf1cb1a06bc44416` for its design-only boundary. Its authority roster depends on a production-verified, exact D9.0.1 19-role generation plus one separately versioned, design-only human binding used only as the second external roster adopter; it does not weaken human gates or grant operational authority. Approval leaves the fingerprinted `design_only_contract_freeze` state and contract root unchanged. Resolver revisions 1.0.0, 1.1.0 and 1.2.0 are approved design-only freezes. Their combined D9.4 route has a confirmed identity conflict because an independently derived state-head digest cannot also be the exact journal-tip receipt digest. The additive [source-head/receipt identity correction 1.3.0](d9-recovery-source-head-identity-v1-3.md) is an unapproved proposal that keeps both identities distinct and binds them through authenticated correspondence without modifying frozen bytes. Resolver outputs remain technical recovery evidence only and grant no runtime or recovery authority, evidence acceptance, legal verification, deletion, restoration, publication eligibility, operational permission, or D9.5 authority. D9.4.1 remains stopped; D9.5 remains unimplemented and separately reviewed.
