...
- Implement and separately review D9.3.1 as the next approval boundary using the approved [D9.3.0 custody-durability contract freeze](d9-3-0-custody-durability-contracts.md), which does not alter D9.0.1 and authorizes no recovery action or runtime use. The remaining operational Decision 9 sequence still requires real ceremony authorization, authenticated-principal activation, D9.1/D9.2 process integration, local custody adapter, clearance procedure, D9.4 restriction/tombstone controls, D9.5 backup/restore tests, and durable operational audit before accepting any evidence.
