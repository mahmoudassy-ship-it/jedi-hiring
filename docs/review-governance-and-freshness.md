...
Status: approved sixth architectural requirement. Separately numbered implementation Decision 6 approves the boundary between Tranche 2A source-evidence quarantine and Tranche 2B source-backed authority drafting, Decision 7 approves the hybrid pilot evidence-custody/import contract, and Decision 8 approves six logical Tranche 2A evidence boundaries; the architectural-requirement and implementation-decision numbering series are distinct. The exact 2A schema and manifest contract were later approved, and migration `005_tranche_2a_source_quarantine.sql` implements the empty nine-table physical schema. [Decision 9](tranche-2a-importer-custody.md) approves the operational architecture for the fixed-function importer, identity boundary, custody, backup and one-document pilot as proposed at commit `ee3ccd777849c7ffcad1efe5d5d9170c15171a65`. Its design-only [D9.0 contract freeze](d9-0-contract-freeze.md) was approved at commit `320b2d6969ede88796c44e652f6422f73e7fe4fe`; proposed D9.0.1 is an explicit least-privilege successor that preserves that baseline in Git history and awaits separate approval. The corrected set contains only machine-readable contracts, registries, synthetic fixtures, and an offline validator. It specifies 19 distinct runtime bindings, explicit bootstrap and recovery permits, exact global and operation-scoped handle partitions, total operational state matrices, deterministic digest/ordering profiles, and exact limits, but implements none of those controls. No importer, collector, artifact adapter, authentication/authorization binding, monitoring, review-domain table, operational safeguard, evidence, or public surface exists yet. Migration implementation and design approval are not authorization for real evidence, public exposure, or production use.
...
- After separate approval of the applicable D9 contract revision, implement and security-review the remaining Decision 9 sequence: fixed-function importer, first-bundle ceremony, authenticated-principal bindings, non-SQL write isolation, local custody adapter, clearance procedure, retention/tombstone controls, backup/restore tests and durable operational audit before accepting any evidence. D9.1 may test only its synthetic control-plane no-effect path; complete accepted-bundle projection/no-op verification remains D9.2. Any D9.3 durability-receipt contract extension must be separately versioned, reviewed, and approved; it must not silently alter an approved D9.0 revision.
