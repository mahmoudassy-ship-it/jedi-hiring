...
- Preserve the approved design-only [D9.4.0 restriction/deletion contract freeze](d9-4-restriction-deletion-contracts.md), the approved synthetic, unactivated [D9.3.1 primary-custody implementation](d9-3-1-primary-custody-runtime.md), and the approved design-only [recovery-projection resolver contract](d9-recovery-projection-resolvers.md) revision `1.0.0`. None executes recovery, accepts real evidence, authorizes restriction or deletion, establishes legal verification or publication eligibility, or activates runtime use. D9.4.1 remains stopped and requires a separate implementation review. The remaining operational Decision 9 sequence still requires real ceremony authorization, authenticated-principal activation, full D9.1/D9.2/D9.3 process integration, D9.5 backup/restore tests, and durable operational audit before accepting any evidence.
