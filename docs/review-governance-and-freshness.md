...
- Obtain separate approval for D9.0's proposed closed contracts, including the distinct human `recovery_authority`, exact-operation recovery permit, concrete post-bootstrap logical-state commitment, total clearance/custody/journal/result matrices, digest profiles, and bounded IPC replay state. Then implement and security-review the remaining Decision 9 sequence: fixed-function importer, first-bundle ceremony, authenticated-principal bindings, non-SQL write isolation, local custody adapter, clearance procedure, retention/tombstone controls, backup/restore tests and durable operational audit before accepting any evidence.
