...
- Preserve the approved frozen and synthetic boundaries through D9.4.1. The D9.5.0 proposal now defines, but does not approve or implement, exact-byte backup evidence, semantically matched D9.3 artifact references, explicit D9.5 source-copy receipts, authenticated three-human approval submissions, deletion-aware reconstruction, reverse-complete current-control mapping, fresh protected retention-head attestations at planning and promotion, independently verified atomic promotion, deterministic drills, and closed crash classification. Its fixtures and storage descriptions remain synthetic; operational identities, storage, credentials, backup media, trusted clocks, external rollback resistance, and recovery authority remain prerequisites.
