...
Current D9.4 note: the [D9.4.0 restriction/deletion contract freeze](d9-4-restriction-deletion-contracts.md) is approved at reviewed commit `7db408fb0f9083f4a17c89cbaf1cb1a06bc44416` for its design-only boundary. Its authority roster depends on a production-verified, exact D9.0.1 19-role generation plus one separately versioned, design-only human binding used only as the second external roster adopter; it does not weaken human gates or grant operational authority. Approval leaves the fingerprinted `design_only_contract_freeze` state and contract root unchanged, creates no restriction or deletion effect, and does not establish accepted evidence or verified legal authority. The [D9 recovery-projection resolver contract](d9-recovery-projection-resolvers.md) is an unapproved design proposal for bounded, authenticated technical snapshot messages; it grants no recovery action and does not resume D9.4.1. D9.4.1 and D9.5 remain unimplemented and separately reviewed.
...
- Preserve the approved design-only [D9.4.0 restriction/deletion contract freeze](d9-4-restriction-deletion-contracts.md) and the approved synthetic, unactivated [D9.3.1 primary-custody implementation](d9-3-1-primary-custody-runtime.md). Neither executes recovery, accepts real evidence, authorizes restriction or deletion, or activates runtime use. The separately versioned [recovery-projection resolver proposal](d9-recovery-projection-resolvers.md) must be approved before D9.4.1 may resume; even if approved it would freeze technical messages only. The remaining operational Decision 9 sequence still requires real ceremony authorization, authenticated-principal activation, full D9.1/D9.2/D9.3 process integration, D9.5 backup/restore tests, and durable operational audit before accepting any evidence.
