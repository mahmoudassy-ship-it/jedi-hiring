...
Status: approved sixth architectural requirement. Separately numbered implementation Decision 6 approves the boundary between Tranche 2A source-evidence quarantine and Tranche 2B source-backed authority drafting, Decision 7 approves the hybrid pilot evidence-custody/import contract, and Decision 8 approves six logical Tranche 2A evidence boundaries; the architectural-requirement and implementation-decision numbering series are distinct. The exact 2A schema and manifest contract were later approved, and migration `005_tranche_2a_source_quarantine.sql` implements the empty nine-table physical schema. [Decision 9](tranche-2a-importer-custody.md) is an unapproved operational design proposal for the fixed-function importer, identity boundary, custody, backup and one-document pilot. No importer, collector, artifact adapter, authentication/authorization binding, monitoring, review-domain table, operational safeguard, evidence, or public surface exists yet. Migration implementation is not authorization for real evidence, public exposure, or production use.
...
- Approve, implement and security-review [Decision 9](tranche-2a-importer-custody.md), including the fixed-function importer, first-bundle ceremony, authenticated-principal bindings, non-SQL write isolation, local custody adapter, clearance procedure, retention/tombstone controls, backup/restore tests and durable operational audit, before accepting any evidence.
