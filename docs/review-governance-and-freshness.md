...
Current D9.4 note: the [D9.4.0 restriction/deletion contract freeze](d9-4-restriction-deletion-contracts.md) is approved at reviewed commit `7db408fb0f9083f4a17c89cbaf1cb1a06bc44416` for its design-only boundary. Its authority roster depends on a production-verified, exact D9.0.1 19-role generation plus one separately versioned, design-only human binding used only as the second external roster adopter; it does not weaken human gates or grant operational authority. Approval leaves the fingerprinted `design_only_contract_freeze` state and contract root unchanged, creates no restriction or deletion effect, and does not establish accepted evidence or verified legal authority. The [D9 recovery-projection resolver contract](d9-recovery-projection-resolvers.md) revision `1.0.0`, [durability extension 1.1.0](d9-recovery-resolver-durability-extension.md), and [checkpoint progression correction 1.2.0](d9-recovery-checkpoint-progression-v1-2.md) are approved design-only freezes. The additive [source-head/receipt identity correction 1.3.0](d9-recovery-source-head-identity-v1-3.md) is approved externally at reviewed commit `d9eddd21494f59e27b2b6365456b53ee2719db2c` as a design-only contract freeze. It resolves the combined route's identity conflict by keeping the independently derived source-state identity and exact journal-tip receipt identity distinct and binding them through authenticated correspondence; it supersedes only that conflicting route and does not reinterpret or modify frozen v1, v1.1, or v1.2 artifacts. Resolver outputs are technical recovery evidence only and grant no runtime or recovery authority, evidence acceptance, legal verification, deletion, restoration, publication eligibility, operational permission, or D9.5 authority. D9.4.1 remains a synthetic, unactivated and unapproved implementation candidate under v1.1–v1.3 reconciliation; D9.5 remains unimplemented and separately reviewed.

Status: approved sixth architectural requirement. Separately numbered implementation Decision 6 approves the boundary between Tranche 2A source-evidence quarantine and Tranche 2B source-backed authority drafting, Decision 7 approves the hybrid pilot evidence-custody/import contract, and Decision 8 approves six logical Tranche 2A evidence boundaries; the architectural-requirement and implementation-decision numbering series are distinct. The exact 2A schema and manifest contract were later approved, and migration `005_tranche_2a_source_quarantine.sql` implements the empty nine-table physical schema. [Decision 9](tranche-2a-importer-custody.md) approves the operational architecture for the fixed-function importer, identity boundary, custody, backup and one-document pilot as proposed at commit `ee3ccd777849c7ffcad1efe5d5d9170c15171a65`. Its historical design-only [D9.0 contract freeze](d9-0-contract-freeze.md) was approved at commit `320b2d6969ede88796c44e652f6422f73e7fe4fe`; D9.0.1 was approved at commit `58a10d43ee5fcc3121a368301b4f5a925f26a9e3` and is the applicable revision for subsequent D9 implementation, superseding only the corrected runtime-profile and handle-scope contracts. D9.1 is the approved synthetic, unactivated control plane. The [synthetic, unactivated D9.2 accepted-bundle verifier](d9-2-accepted-bundle-verifier.md) is approved at implementation commit `460547b7fe75a94f689dabc97fd91ee6f803934a`. It verifies technical correspondence between an accepted quarantine bundle and disposable persisted evidence; it does not verify legal authority. No production activation follows: no collector, operational custody adapter, authentication/authorization binding, monitoring, review-domain table, real evidence, legal verification, publication, API, frontend, or public surface exists. The corrected design-only [D9.3.0 custody-durability contract freeze](d9-3-0-custody-durability-contracts.md) is approved externally at reviewed commit `89433cf058ca8f52b658ea1ce2d0391aaf44f7fe`; its fingerprinted `design_only_contract_freeze` state remains unchanged and approval creates no runtime authority. The [synthetic, unactivated D9.3.1 primary-custody implementation](d9-3-1-primary-custody-runtime.md) is approved at commit `d960a03f4b8ac8a78d3f6b40ba909eb24b5f3442`. The design-only [D9.4.0 restriction/deletion contract freeze](d9-4-restriction-deletion-contracts.md) is approved externally at reviewed commit `7db408fb0f9083f4a17c89cbaf1cb1a06bc44416`; its fingerprinted state remains unchanged and no runtime is activated. The D9.4.1 synthetic implementation candidate remains unapproved and unactivated. Physical custody or a restriction/deletion design or synthetic runtime cannot establish evidence acceptance, officiality, legal authority, human review, or publication eligibility. D9.5 remains separately reviewed and unimplemented.
...
- Preserve the approved design-only [D9.4.0 restriction/deletion contract freeze](d9-4-restriction-deletion-contracts.md), the approved synthetic, unactivated [D9.3.1 primary-custody implementation](d9-3-1-primary-custody-runtime.md), the approved design-only [recovery-projection resolver contract](d9-recovery-projection-resolvers.md) revision `1.0.0`, its approved design-only [1.1.0 durability extension](d9-recovery-resolver-durability-extension.md), its approved design-only [1.2.0 checkpoint-progression correction](d9-recovery-checkpoint-progression-v1-2.md), its approved design-only [1.3.0 source-head/receipt-identity correction](d9-recovery-source-head-identity-v1-3.md), and the unapproved [D9.4.1 synthetic restriction/deletion implementation candidate](d9-4-1-restriction-deletion-runtime.md). None accepts real evidence, activates runtime use, establishes legal authority, executes recovery, or grants publication eligibility. D9.4.1 now executes closed synthetic resolver projections, an empty-only protected source-head checkpoint bootstrap, canonical component/composite resolver custody, append-time head revalidation, a historical `1.2.0` reader with no new v1.2 progression writes, and complete protected `1.3.0` correspondence and checkpoint progression. The v1.3 path separately derives source-state identity, retains the actual journal-tip receipt identity, and binds them through authenticated correspondence; a receipt digest cannot satisfy a state-head CAS. Assessment links, restart reconstruction, exact replay, and fail-closed reconciliation remain technical recovery evidence only. Its D9.1/D9.2/D9.3 adapters and durability store remain fixture-backed disposable test boundaries rather than operational protected-source readers or storage. It performs recovery classification only and can unlink only an exact synthetic primary name inside its disposable test root; it authorizes no real restriction or deletion. D9.5 remains the next separate backup/restore and deletion-aware-reconstruction milestone. The remaining operational Decision 9 sequence still requires real ceremony authorization, authenticated-principal activation, operational D9.1/D9.2/D9.3 process integration, D9.5 tests, protected operational storage, external rollback resistance, and durable operational audit before accepting any evidence.
