...
- Preserve the approved design-only [D9.4.0 restriction/deletion contract freeze](d9-4-restriction-deletion-contracts.md), the approved synthetic, unactivated [D9.3.1 primary-custody implementation](d9-3-1-primary-custody-runtime.md), the approved design-only [recovery-projection resolver contract](d9-recovery-projection-resolvers.md) revision `1.0.0`, and the unapproved [D9.4.1 synthetic restriction/deletion implementation candidate](d9-4-1-restriction-deletion-runtime.md). None accepts real evidence, activates runtime use, establishes legal authority, executes recovery, or grants publication eligibility. D9.4.1 now executes closed synthetic resolver projections and append-time head revalidation, but its D9.1/D9.2/D9.3 adapters remain fixture-backed test boundaries rather than operational protected-source readers. It performs recovery classification only and can unlink only an exact synthetic primary name inside its disposable test root; it authorizes no real restriction or deletion. D9.5 remains the next separate backup/restore and deletion-aware-reconstruction milestone. The remaining operational Decision 9 sequence still requires real ceremony authorization, authenticated-principal activation, operational D9.1/D9.2/D9.3 process integration, D9.5 tests, and durable operational audit before accepting any evidence.
