...
Current D9.4 note: the [D9.4.0 restriction/deletion contract freeze](d9-4-restriction-deletion-contracts.md) is approved at reviewed commit `7db408fb0f9083f4a17c89cbaf1cb1a06bc44416` for its design-only boundary. Its authority roster depends on a production-verified, exact D9.0.1 19-role generation plus one separately versioned, design-only human binding used only as the second external roster adopter; it does not weaken human gates or grant operational authority. Approval leaves the fingerprinted `design_only_contract_freeze` state and contract root unchanged, creates no restriction or deletion effect, and does not establish accepted evidence or verified legal authority. D9.4.1 is implemented only as a synthetic, unactivated candidate awaiting separate approval; D9.5 remains unimplemented and separately reviewed.
...
- Preserve the approved design-only [D9.4.0 restriction/deletion contract freeze](d9-4-restriction-deletion-contracts.md), the approved synthetic, unactivated [D9.3.1 primary-custody implementation](d9-3-1-primary-custody-runtime.md), and the unapproved [D9.4.1 synthetic restriction/deletion implementation candidate](d9-4-1-restriction-deletion-runtime.md). None accepts real evidence, activates runtime use, or establishes legal authority. D9.4.1 performs recovery classification only and can unlink only an exact synthetic primary name inside its disposable test root; it authorizes no real restriction or deletion. D9.5 remains the next separate backup/restore and deletion-aware-reconstruction milestone. The remaining operational Decision 9 sequence still requires real ceremony authorization, authenticated-principal activation, full D9.1/D9.2/D9.3 process integration, D9.5 tests, and durable operational audit before accepting any evidence.
