...
- Implement and security-review the remaining Decision 9 sequence against the approved D9.0 contracts: fixed-function importer, first-bundle ceremony, authenticated-principal bindings, non-SQL write isolation, local custody adapter, clearance procedure, retention/tombstone controls, backup/restore tests and durable operational audit before accepting any evidence. Any D9.3 durability-receipt contract extension must be separately versioned, reviewed, and approved; it must not silently alter the frozen D9.0 version-1 contract.
