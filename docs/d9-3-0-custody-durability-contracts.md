...
2. **D9.3.1 — primary custody vertical slice:** the [synthetic implementation candidate](d9-3-1-primary-custody-runtime.md) supplies the restricted local CAS, adapter IPC/descriptor mechanics, receipt and journal brokers, copy-aware integrity relay, D9.0.1 admission/resolution integration, protected inventory-snapshot consumer, and classification-only crash handling. It awaits separate approval, cannot execute a recovery action, and remains unactivated.
