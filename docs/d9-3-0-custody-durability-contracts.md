...
3. **D9.4.0 — approved design-only restriction/deletion contract freeze:** append-only controls, access shutdown, separation of duty, bounded primary-copy deletion receipts, classification-only recovery, and D9.5 directives; no runtime effect. **D9.4.1** is separately approved only as a synthetic, unactivated implementation; D9.5 remains unimplemented.
