...
3. **D9.2 — next separately reviewed step, accepted-bundle projection verification and isolated DB writer:** secure manifest opening, approved v1 parser/canonicalizer, complete preflight, typed plan, fixed statements, disposable candidate cloning, full projection and independent promotion verifier; complete accepted-bundle no-op verification for both bundle kinds; separately exercise from-zero reconstruction while all bytes exist.
