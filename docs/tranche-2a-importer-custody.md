...
4. **D9.3.0 — approved design-only contract freeze:** separately versioned primary-receipt, adapter-v2, journal-v2 and conservative crash-state classification contracts only; no runtime or recovery action. **D9.3.1** now has a [synthetic, unactivated implementation candidate](d9-3-1-primary-custody-runtime.md) awaiting separate approval; it does not enable operational custody or recovery action.
