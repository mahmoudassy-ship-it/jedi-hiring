...
Status: **approved at commit `ee3ccd777849c7ffcad1efe5d5d9170c15171a65`; not authorized for real evidence**. The approved version-1 manifest contract and the empty migration-005 schema are unchanged. This decision specifies the operational boundary required before either may be used. It does not authorize credentials, evidence acquisition, principal creation, an import, public exposure, or production use. The historical design-only [D9.0 contract freeze](d9-0-contract-freeze.md) was approved at commit `320b2d6969ede88796c44e652f6422f73e7fe4fe`; the explicit D9.0.1 least-privilege correction was approved at commit `58a10d43ee5fcc3121a368301b4f5a925f26a9e3` and is the applicable revision for subsequent D9 implementation. It preserves that baseline in Git history and supersedes only the corrected runtime-profile and handle-scope contracts. The corrected set supplies closed machine-readable runtime, identity, handoff/seal, bootstrap/recovery, custody, clearance, journal, state-seal, result, classification, digest-profile, and field-provenance contracts. D9.1 is approved at implementation commit `79046c27dfa6923383be33a1d0aaa5a7641f5d1f` only for its synthetic, unactivated launcher/bootstrap control boundary. The [synthetic, unactivated D9.2 accepted-bundle verifier](d9-2-accepted-bundle-verifier.md) is approved at implementation commit `460547b7fe75a94f689dabc97fd91ee6f803934a` for secure manifest intake, semantic preflight, deterministic planning, isolated disposable writing, full projection, accepted no-op, and reconstruction verification. Accepted evidence projection does not verify legal identity, officiality, authority, legal status, effect, or a legal proposition. Neither D9.1 nor D9.2 is production-ready or authorizes real bootstrap, evidence import, canonical promotion, custody operations, credentials, legal verification, publication, API use, or frontend use. The corrected design-only [D9.3.0 custody-durability contract freeze](d9-3-0-custody-durability-contracts.md) is approved externally at reviewed commit `89433cf058ca8f52b658ea1ce2d0391aaf44f7fe`; approval leaves its fingerprinted contracts unchanged and authorizes no runtime use. D9.3.1 remains the next separately reviewed, unstarted implementation step.
...
- full semantic preflight before `BEGIN`, atomic SQLite insertion, complete nine-table projection comparison before `COMMIT` and during replay, and fail-closed replay rules;
...
Decision 9 approves the architecture and sequence below only. D9.1 is approved only as a synthetic, unactivated control plane; its approval neither authorizes activation nor makes it production-ready. The historical design-only [D9.0 contract freeze](d9-0-contract-freeze.md) was approved at commit `320b2d6969ede88796c44e652f6422f73e7fe4fe`; D9.0.1 was separately approved at commit `58a10d43ee5fcc3121a368301b4f5a925f26a9e3` and is the applicable least-privilege successor only for the corrected runtime-profile and handle-scope contracts. Each implementation tranche still ends with synthetic-only tests and its own approval boundary.

1. **D9.0/D9.0.1 — approved contract freeze and explicit correction:** versioned runtime, identity-binding, handoff/bundle-seal, bootstrap/recovery, custody, clearance, journal, logical-state-seal and result schemas; total transition/outcome matrices; exact digest/field registries and deterministic ordering; exact file/path/IPC/resource/scanner/time limits; synthetic fixtures; and the approved corrected operation-handle partitions. The original approved revision remains in Git history. No executable control, credentials or real data.
2. **D9.1 — approved synthetic, unactivated control plane:** contract/build and active-generation checks; a dedicated-UID append worker for the disposable private-pipe harness; protected handoff/seal and unclaimable permit state; exact seven-partition scope enforcement; separately exercised kernel peer/descriptor/lock primitives; empty-state checks; and a synthetic zero-handle control-plane dry-run/rollback/no-effect test. The operational `AF_UNIX` service endpoints, permit claim, permitted-operation grant issuance, accepted-bundle projection and importer no-op verification remain absent. Approval is not production readiness and does not authorize real bootstrap, evidence import, custody operations, database writing, or legal verification.
3. **D9.2 — approved synthetic, unactivated accepted-bundle verifier at implementation commit `460547b7fe75a94f689dabc97fd91ee6f803934a`:** secure manifest opening, approved v1 parser/canonicalizer, complete preflight, typed plan, fixed statements limited to disposable candidates, full projection and independent candidate/post-copy verification, complete accepted-bundle no-op verification for both bundle kinds, and from-zero reconstruction while all bytes exist. It performs no operational promotion, canonical write, custody action, real bootstrap, legal verification, publication, API, or frontend effect.
4. **D9.3.0 — approved design-only contract freeze:** separately versioned primary-receipt, adapter-v2, journal-v2 and conservative crash-state classification contracts only; no runtime or recovery action. **D9.3.1** is the next separately reviewed step and may implement primary custody and journal behavior only after its own approval.
...
- Decision 9 remains the approved architecture; D9.0.1 is the applicable approved D9 contract revision, and each relevant later implementation tranche has been separately approved, implemented, independently reviewed, and committed; migration 005 remains unchanged and its existing standalone validator passes.
...
Decision 9, the approved historical D9.0 baseline, and the applicable approved D9.0.1 correction fix design contracts only. D9.0.1 supersedes only the corrected runtime-profile and handle-scope contracts while preserving the original approval historically. Approved D9.2 supplies a synthetic, unactivated verifier and disposable fixed-statement writer, not an operational importer. No real bootstrap ceremony, credential, activated runtime component, custody object, canonical principal, evidence import, legal verification, publication, API, or frontend use is authorized. The separately versioned [D9.3.0 custody-durability contract freeze](d9-3-0-custody-durability-contracts.md) is approved externally at reviewed commit `89433cf058ca8f52b658ea1ce2d0391aaf44f7fe`; its approval creates no runtime and does not establish evidence acceptance or legal authority.
