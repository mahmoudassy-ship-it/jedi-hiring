...
3. **D9.2 — implementation candidate awaiting separate approval:** secure manifest opening, approved v1 parser/canonicalizer, complete preflight, typed plan, fixed statements limited to disposable candidates, full projection and independent candidate/post-copy verification, complete accepted-bundle no-op verification for both bundle kinds, and from-zero reconstruction while all bytes exist. It performs no operational promotion, canonical write, custody action, or real bootstrap.
...
Decision 9, the approved historical D9.0 baseline, and the applicable approved D9.0.1 correction fix design contracts only. D9.0.1 supersedes only the corrected runtime-profile and handle-scope contracts while preserving the original approval historically. No importer, bootstrap ceremony, credential, runtime component, custody object, principal creation, or evidence import is authorized yet. Any D9.3 durability-receipt extension requires its own version, review, and approval.
