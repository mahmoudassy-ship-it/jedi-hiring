...
Historical projection uses both `known_at` and `known_through_receipt_sequence`. Later restrictions, releases, revocations, corrections, tombstones or execution facts do not rewrite a prior bounded view. A tombstone withholds access but does not claim byte deletion. Recovery reconstruction binds the protected head and inventory, but D9.4.0’s control/access/lineage/custody snapshot fields require exact D9.1, D9.2 and D9.3 resolver inputs that are not available in this unactivated slice; it therefore refuses classification rather than inventing projection digests. The frozen recovery schema requires a positive receipt sequence, so an empty protected ledger cannot be classified. Control-only subject history is classified as `restriction_persisted` for future resolver-backed reconstruction, not as an empty/no-op stage.
...
- Recovery is classification-only; a human-authorized recovery executor is absent. Classification proofs are issued only inside the fixed reconstruction/classifier module; there is no public reconstruction registrar or proof issuer. The proof binds the exact record, protected broker, ledger head, inventory digest, and derived snapshot when the separately implemented D9.1/D9.2/D9.3 resolvers are available. A caller cannot mint a current-state proof by registering a fabricated reconstruction. This unactivated slice refuses classification until those exact resolver inputs exist.
