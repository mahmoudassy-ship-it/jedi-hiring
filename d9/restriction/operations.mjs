...
export async function executeSyntheticPrimaryDeletion({
  ...
  await broker.withDestructiveLock(async (lock) => {
    ...
    unlinkAppendResult = await lock.effectThenAppend({ record: unlinkRecord, semanticSession: executorSession, persistenceSession }, async () => {
      ...
      const synced = deleteRuntime.syncParent({ rootPath, artifact, inventory: observed, recordInventory: unlinkRecord.inventory, recordDigestSha256: unlinkRecord.record_digest_sha256, operationNonce: unlinkRecord.operation_nonce, unlinkProof: unlinked.effectProof })
