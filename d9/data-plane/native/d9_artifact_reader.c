...
#define D9_ARTIFACT_BYTES_HARD_MAX 26214400ULL

static void fail(const char *code, int detail) {
  if (detail == 0) (void)fprintf(stderr, "%s\n", code);
  else (void)fprintf(stderr, "%s:%d\n", code, detail);
  exit(2);
}
...
static bool safe_relative_path(const char *value) {
  ...
  if (total_length == 0U || total_length > 512U || value[0] == '/') return false;
  while (*cursor != '\0') {
    const unsigned char byte = *cursor;
    if (byte == '\\' || byte < 0x21U || byte == 0x7fU) return false;
    if (byte == '/') {
      if (component_length == 0U || component_length > 180U) return false;
...
static unsigned long long parse_limit(const char *value) {
  ...
  if (errno != 0 || end == value || *end != '\0' || parsed > D9_ARTIFACT_BYTES_HARD_MAX) fail("INPUT_LIMIT_INVALID", 0);
  return parsed;
}
...
int main(int argc, char **argv) {
  ...
  size_t offset = 0U;
  int descriptor;
  if (argc != 3 || !safe_relative_path(argv[1])) fail("INPUT_ARGUMENT_INVALID", 0);
  expected_length = parse_limit(argv[2]);
  if (fstat(root_fd, &root_status) != 0 || !S_ISDIR(root_status.st_mode)) fail("INPUT_ROOT_INVALID", errno);
  ...
  if (fstat(descriptor, &before) != 0 || !S_ISREG(before.st_mode) || before.st_nlink != 1 || before.st_size < 0 ||
      (unsigned long long)before.st_size != expected_length) {
    (void)close(descriptor);
    fail("INPUT_SIZE_OR_TYPE_REJECTED", errno);
  }
  if (before.st_size > 0) {
    bytes = malloc((size_t)before.st_size);
    if (bytes == NULL) { (void)close(descriptor); fail("RESOURCE_LIMIT_EXCEEDED", 0); }
  }
  while (offset < (size_t)before.st_size) {
    const ssize_t amount = pread(descriptor, bytes + offset, (size_t)before.st_size - offset, (off_t)offset);
    if (amount < 0 && errno == EINTR) continue;
    if (amount <= 0) { free(bytes); (void)close(descriptor); fail("INPUT_READ_FAILED", errno); }
    offset += (size_t)amount;
  }
  if (fstat(descriptor, &after) != 0 || !same_identity(&before, &after)) {
    free(bytes); (void)close(descriptor); fail("INPUT_CHANGED", errno);
  }
  if (close(descriptor) != 0) { free(bytes); fail("INPUT_CLOSE_FAILED", errno); }
  write_all(bytes, (size_t)before.st_size);
  free(bytes);
  return 0;
}
